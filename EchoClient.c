...
char buffer[100] = "My name is 김태정";
char rcvBuffer[100];
...
//1.  서버 소켓 생성
// 서버 소켓 = 클라이언트의 접속을 처리(허용)해주기 위한 소켓
...
if(bind(s_socket,(struct sockaddr*) &s_addr, sizeof(s_addr)) == -1) {
	//바인딩 작업 실패 시 cannot find 메시지 출력 후 프로그램 종료
	printf("Can not Bind\n");
	return -1;
}
...
//5. 클라이언트 요청 처리
// 요청을 허용한 후, Hello World 메세지를 전송
while(1){
	
	len = sizeof(c_addr);
	printf("클라이언트 접속을 기다리는 중...\n");
	c_socket = accept(s_socket,(struct sockaddr*) &c_addr, &len);
	//클라이언트의 요청이 허용( accept) 해 주고, 해당 클라이언트와 통신할 수 있도록 클라이언트 소켓( c_socket) 을 반환함.
